...
const scheduledPurchaseSchema = z.object({
  ...
  if (value.nextRunAt.getTime() <= Date.now()) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['nextRunAt'], message: 'First run must be in the future' });
  }
...
// Direct pay-per-order request. serviceType is strict; the per-type detail
// fields are validated authoritatively server-side in the direct route, so this
// schema only enforces the enum and passes through the (optionally-typed)
// fields the route then re-validates against the live catalogue.
const directOrderSchema = z.object({
  serviceType: z.enum(['airtime', 'data', 'electricity', 'cable', 'exam-pin']),
  network: z.string().trim().optional(),
  phone: z.string().optional(),
  amount: z.number().positive().finite().optional(),
  planCode: z.string().trim().optional(),
  disco: z.string().trim().optional(),
